...
app.post('/api/submit-form', upload.array('attachments'), async (req, res) => {
    ...
    try {
        const result = await dbQuery(
            'INSERT INTO applications ("userEmail", type, data, "paymentStatus", "payment_status") VALUES ($1, $2, $3, $4, $5)',
            [userEmail, type, applicationData, 'Pending', 'Unpaid']
        );
        res.status(200).json({ id: result.rows[0] ? result.rows[0].id : null });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
app.get('/api/operator/applications', async (req, res) => {
    ...
    try {
        ...
        if (search) {
            const searchClause = dbMode === 'pg'
                ? ` WHERE "userEmail" ILIKE $1 OR "type" ILIKE $1 OR CAST(data AS TEXT) ILIKE $1`
                : ` WHERE "userEmail" LIKE $1 OR "type" LIKE $1 OR data LIKE $1`; // SQLite ILIKE is not default
            countQuery += searchClause;
            dataQuery += searchClause;
            params.push(`%${search}%`);
            paramIndex++;
...
>>>>>>> 7276d1c6dc206531c5e51ce117e737d3d41522f5
